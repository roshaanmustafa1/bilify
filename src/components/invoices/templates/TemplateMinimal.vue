    ...
    <div class="flex justify-between items-start mb-6">
      ...
      <div class="w-24 h-24 flex items-center justify-center overflow-hidden">
        ...
        <svg
          ...
          class="w-full h-full"
        >
          <path d="M50 5L90 25V75L50 95L10 75V25L50 5Z" stroke="#4A154B" stroke-width="3" />
          <path d="M50 15L80 30V70L50 85L20 70V30L50 15Z" stroke="#1a2b6d" stroke-width="4" />
          <path d="M40 40L30 50L40 60M60 40L70 50L60 60M45 70L55 30" stroke="#1a2b6d" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </div>
    </div>
      ...
      <div class="bg-primary/10 p-5 rounded-lg">
        ...
        <div class="space-y-1.5 text-foreground">
          ...
          <div v-if="sender.email">
            <span class="font-bold text-foreground">Email :</span> {{ sender.email }}
          </div>
          <div v-if="sender.phone">
            <span class="font-bold text-foreground">Phone :</span> {{ sender.phone }}
          </div>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-primary text-primary-foreground">
            <th class="py-3 px-4 font-normal text-sm border-r border-white/20 w-1/2">Items Detail</th>
            <th class="py-3 px-4 font-normal text-sm text-center border-r border-white/20">Quantity</th>
            <th class="py-3 px-4 font-normal text-sm text-center border-r border-white/20">Rate</th>
            <th class="py-3 px-4 font-normal text-sm text-center">Amount</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in document.items" :key="item.id" :class="index % 2 === 0 ? 'bg-background' : 'bg-primary/10/50'">
            <td class="py-4 px-4">
              <div class="flex">
                <span class="text-muted-foreground mr-2">{{ String(index + 1).padStart(2, "0") }}.</span>
                <div>
            ...
            <td class="py-4 px-4 text-center text-foreground">{{ String(item.quantity).padStart(2, "0") }}</td>
            <td class="py-4 px-4 text-center text-foreground">{{ formatCurrency(item.price) }}</td>
            <td class="py-4 px-4 text-center text-foreground">{{ formatCurrency(item.quantity * item.price) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="grid grid-cols-2 gap-12 mb-12">
      <div class="bg-primary/10 p-5 rounded-lg h-fit relative">
        ...
        <div class="space-y-1.5 text-foreground mt-3">
          <div><span class="font-bold text-foreground">Account Holder Name :</span> {{ bank.accountName || sender.name }}</div>
          <div><span class="font-bold text-foreground">Account Number :</span> {{ bank.accountNumber || "N/A" }}</div>
          <div><span class="font-bold text-foreground">IBAN :</span> {{ bank.iban || "N/A" }}</div>
          <div><span class="font-bold text-foreground">Bank :</span> {{ bank.bankName || "N/A" }}</div>
        </div>
      </div>
      ...
      <div>
        <div class="flex justify-between border-t border-b border-border py-3 mb-6">
          ...
          <span class="text-foreground font-medium text-base">{{ formatCurrency(document.total) }}</span>
        </div>

        <div>
          <p class="font-bold text-foreground text-sm mb-3">Terms and Conditions :</p>
        ...
        <div v-if="document.notes" class="mt-4">
          ...
          <div class="text-xs text-foreground whitespace-pre-line">{{ document.notes }}</div>
        </div>
      </div>
    </div>
