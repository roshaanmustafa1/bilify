<template>
  <div class="ai-invoice-parser bg-card border border-border shadow-sm rounded-lg p-6">
    <div class="mb-4">
      ...
      <p class="text-sm text-muted-foreground mt-2">
        Paste raw invoice text or instructions below. The AI will automatically extract and structure the data.
      </p>
    </div>

    <div class="space-y-4">
      <div>
        <label for="rawText" class="block text-sm font-medium text-foreground mb-1">Raw Text Data</label>
        <Textarea 
          id="rawText" 
          v-model="rawText" 
      ...
      <div v-if="error" class="p-3 bg-destructive/10 border border-destructive/20 rounded-md text-sm text-destructive flex items-start gap-2">
...
export default {
  ...
  methods: {
    ...
    parseInvoice() {
      ...
      const payload = {
        prompt: "Extract billing parameters into structured invoice JSON schema",
      ...
      axios.post('https://api.openai.com/v1/chat/completions', payload, {
        headers: {
          ...
          'Authorization': `Bearer ${import.meta.env.VITE_OPENAI_API_KEY || ''}` // Mocked key setup
        }
      })
      .then(response => {
      ...
      .catch(error => {
        this.isParsing = false
        this.error = error.response?.data?.error?.message || error.message || 'An error occurred during AI invoice parsing.'
      })
    }
  }
}
